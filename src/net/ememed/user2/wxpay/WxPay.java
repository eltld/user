...
//			dialog = ProgressDialog.show(mContext, "提示", "正在获取access token...");
			dialog = ProgressDialog.show(mContext, "提示", "正在连接微信服务器...");
		}
...
//				Toast.makeText(mContext, "获取access token成功", Toast.LENGTH_LONG).show();
...
//				Toast.makeText(mContext, "获取prepayid成功", Toast.LENGTH_LONG).show();
		...
		protected GetPrepayIdResult doInBackground(Void... params) {
			...
			String entity = genProductArgs();// post参数
	...
	private String genProductArgs() {
		...
		try {
			...
			nonceStr = genNonceStr();// 32位随机串
			json.put("noncestr", nonceStr);

			List<NameValuePair> packageParams = new LinkedList<NameValuePair>();
			packageParams.add(new BasicNameValuePair("bank_type", "WX"));
			packageParams.add(new BasicNameValuePair("body", body));// 商品描述
			packageParams.add(new BasicNameValuePair("fee_type", FEE_TYPE));// 币种，1人民币
			packageParams.add(new BasicNameValuePair("input_charset", "UTF-8"));
			packageParams.add(new BasicNameValuePair("notify_url", Constants.NOTIFY_URL + orderNo));// 接收财付通通知的URL
			packageParams.add(new BasicNameValuePair("out_trade_no", orderNo));// 商家订单号
			packageParams.add(new BasicNameValuePair("partner", Constants.PARTNER_ID));// 商户号partnerid(10位)
			packageParams.add(new BasicNameValuePair("spbill_create_ip", "196.168.1.1"));// 用户的公网ip，不是商户服务器IP
			packageParams.add(new BasicNameValuePair("total_fee", String.valueOf((int) (Double.valueOf(price) * 100))));// 商品金额,以分为单位(money*100).ToString()
			packageValue = genPackage(packageParams);

			json.put("package", packageValue);// 1、需要生成
			...
			json.put("app_signature", genSign(signParams));// 2、需要生成
	...
	private final String FEE_TYPE = "1";// 现金支付币种,目前只支持人民币,默认值是 1-人民币
	private String body;//
	...
	public void pay() {
		if (!isPaySupported()) {
			Toast.makeText(mContext, "当前微信版本不支持支付功能，请更新微信到5.0以上版本", Toast.LENGTH_SHORT).show();
		}

		if (toggle) {
			new GetAccessTokenTask().execute();
		}else {
			Toast.makeText(mContext, "微信支付功能暂未启用", Toast.LENGTH_SHORT).show();
		}
	}
}
