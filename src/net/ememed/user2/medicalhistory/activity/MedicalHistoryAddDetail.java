...
public class MedicalHistoryAddDetail extends BasicActivity implements Callback {
	...
	private ArrayList<String> imagePaths = new ArrayList<String>();
	private ArrayList<ArrayList<String>> imagePathList = new ArrayList<ArrayList<String>>();
			...
			if (initDatas.size() != 0) {
				for (int i = 0; i < initDatas.size(); i++) {
					if (initDatas.get(i) != null && !initDatas.get(i).equals("")) {
						imagePaths.add(initDatas.get(i));
					}
				}
			}
		...
		blGrid.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int point, long id) {
				if (point == (blDrawable.size() - 1)) {
					...
					bundle.putStringArrayList("urlPaths", imagePaths);
					bundle.putInt("point", point);
					Utils.startActivity(MedicalHistoryAddDetail.this, CheckImageActivity.class, bundle);
				}
			}
		});
		blGrid.setOnItemLongClickListener(new OnItemLongClickListener() {
			...
			public boolean onItemLongClick(AdapterView<?> parent, View view, final int point, long id) {
				if (point == blDrawable.size()) {
					...
					final Context dialogContext = new ContextThemeWrapper(MedicalHistoryAddDetail.this, android.R.style.Theme_Light);
		...
		try {
			...
			case IResult.UPLOAD_IMG:
				...
				if (success == 1) {
					...
					imagePaths.add((String) map.get("ATTACHMENT"));
